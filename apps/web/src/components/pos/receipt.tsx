...
interface ReceiptProps {
  ...
  /** Lebar kertas thermal dalam mm (default 58mm). */
  widthMm?: 58 | 80;
}

export const Receipt = React.forwardRef<HTMLDivElement, ReceiptProps>(
  ({ transaction, widthMm = 58 }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className="bg-white p-2 text-black text-sm print:m-0 print:shadow-none"
        style={{ fontFamily: 'monospace', width: `${widthMm}mm` }}
      >
        {/* Header */}
        <div className="text-center mb-4 border-b border-dashed border-gray-400 pb-2">
          <h2 className="font-bold text-lg">{user?.outlet_nama || 'MrikiPOS'}</h2>
